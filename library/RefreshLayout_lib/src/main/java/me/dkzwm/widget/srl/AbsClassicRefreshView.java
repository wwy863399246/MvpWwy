...
public abstract class AbsClassicRefreshView<T extends IIndicator> extends RelativeLayout implements IRefreshView<T>, LastUpdateTimeUpdater.ITimeUpdater
{
    ...
    public AbsClassicRefreshView(Context context, AttributeSet attrs, int defStyleAttr)
    {
        ...
        if (attrs != null)
        {
            final TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.AbsClassicRefreshView, 0, 0);
            @RefreshViewStyle int style = arr.getInt(R.styleable.AbsClassicRefreshView_sr_style, mStyle);
            mStyle = style;
            arr.recycle();
        }
        ...
        {
            mProgressBar.setIndeterminateTintList(ColorStateList.valueOf(ContextCompat.getColor(context, R.color.FD9B21)));
        }
