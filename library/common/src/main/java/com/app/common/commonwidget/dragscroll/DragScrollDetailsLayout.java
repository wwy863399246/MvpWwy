package com.app.common.commonwidget.dragscroll;
...
public class DragScrollDetailsLayout extends LinearLayout
{
    ...
    private boolean checkCanInterceptTouchEvent(MotionEvent ev)
    {
        ...
        {
            mInitialInterceptY = (int) ev.getY();
            if (Math.abs(yDiff) > mTouchSlop && Math.abs(yDiff) >= Math.abs(xDiff) && !(mCurrentViewIndex == CurrentTargetIndex.UPSTAIRS && yDiff > 0 || 
    ...
    {
        if (mCurrentViewIndex == CurrentTargetIndex.UPSTAIRS && event.getY() > mDownMotionY || mCurrentViewIndex == CurrentTargetIndex.DOWNSTAIRS && event
                .getY() < mDownMotionY)
        {
    ...
    private void scroll(MotionEvent event)
    {
        if (mCurrentViewIndex == CurrentTargetIndex.UPSTAIRS)
        {
            ...
            if (distance <= mUpstairsView.getMeasuredHeight())
            {
                scrollTo(0, distance);
            }
        }
        else
        {
            ...
            int distance = event.getY() <= mInitialInterceptY ? mUpstairsView.getMeasuredHeight() : (int) (mInitialInterceptY - event.getY() + mUpstairsView
                    .getMeasuredHeight());
            if (distance >= 0)
            {
                scrollTo(0, distance);
    ...
    {
        void onScrollChanged(DragScrollDetailsLayout scrollView, int x, int y, int oldx, int oldy);
    }
