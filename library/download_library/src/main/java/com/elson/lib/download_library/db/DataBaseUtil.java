...
public class DataBaseUtil
{
    public synchronized static boolean insertDown(Context context, DownLoadBean bean)
    {
        ...
        /** 字段名对应字段值 **/
        String[] titles = new String[]{Constants.DOWN_ID,//
                Constants.DOWN_NAME,//
                Constants.DOWN_ICON,//
                Constants.DOWN_URL,//
    ...
    public synchronized static DownLoadBean getDownLoadById(Context context, String DownloadID)
    {
        ...
        if (cursor.moveToNext())
        {
            ...
            bean.fileName = cursor.getString(cursor.getColumnIndex(Constants.DOWN_NAME));
            bean.fileIcon = cursor.getString(cursor.getColumnIndex(Constants.DOWN_ICON));
            ...
            bean.isSupportRange = cursor.getLong(cursor.getColumnIndex(Constants.DOWN_SUPPORT_RANGE)) != 0;
        }
        cursor.close();
        return bean;
    }
     ...
     * 删除下载数据库数据
     */
    public synchronized static void deleteAll(Context context)
    {
        DBHelper.deleteAll(context, Constants.TABLE_DOWN);
    }
}
