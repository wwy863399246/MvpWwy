...
public class RichTextEditor extends ScrollView
{
    ...
    private static final int EDIT_PADDING = 8; // edittext常规padding是10dp
...
//    private int rtTextHintColor = Color.parseColor("#B5B5C0");
...
//        rtTextSize = ta.getInteger(R.styleable.RichTextView_rt_view_text_size, 16);
        rtTextColor = ta.getColor(R.styleable.RichTextEditor_rt_editor_text_color, Color.parseColor("#323232"));
        rtTextInitHint = ta.getString(R.styleable.RichTextEditor_rt_editor_text_init_hint);
//        rtTextHintColor = ta.getColor(R.styleable.RichTextEditor_rt_editor_text_hint_color,Color.parseColor("#B5B5C0"));
        ...
        allLayout.setPadding(32, 15, 32, 15);//设置间距，防止生成图片时文字太靠边，不能用margin，否则有黑边
        ...
        if (startSelection == 0)
        {
            ...
            if (null != preView)
            {
                ...
                else if (preView instanceof EditText)
                {
                    ...
                    // 文本合并
                    preEdit.setText(SpanStringUtils.getEmotionContent(EmotionUtils.EMOTION_CLASSIC_TYPE, getContext(), preEdit, str1 + str2));
    ...
    private void onImageCloseClick(View view)
    {
        ...
        //删除文件夹里的图片
        List<EditData> dataList = buildEditData(false);
...
//				.start(activity);
    ...
    public EditText createEditText(String hint, int paddingTop)
    {
        ...
        editText.setHintTextColor(Color.parseColor("#B5B5C0"));
    ...
    public void insertImage(Bitmap bitmap, String imagePath)
    {
        ...
        else
        {
            ...
            lastFocusEdit.setText(SpanStringUtils.getEmotionContent(EmotionUtils.EMOTION_CLASSIC_TYPE, getContext(), lastFocusEdit, editStr1));
    ...
    {
        EditText editText2 = createEditText("编辑文字说明", dip2px(getContext(), EDIT_PADDING));
        //判断插入的字符串是否为空，如果没有内容则显示hint提示信息
        if (editStr != null && editStr.length() > 0)
        {
            editText2.setText(SpanStringUtils.getEmotionContent(EmotionUtils.EMOTION_CLASSIC_TYPE, getContext(), editText2, editStr.toString()));
        }
    ...
    public void addImageViewAtIndex(final int index, Bitmap bmp, String imagePath)
    {
        ...
        ImageLoaderUtils.displayRoundedCorners(getContext(), imageView, imagePath);
//        GlideApp.with(getContext()).load(imagePath).centerCrop().into(imageView);
        ...
        RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, rtImageHeight);//固定图片高度500，考虑自定义属性
...
//		}
        ...
        RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, rtImageHeight);//固定图片高度，记得设置裁剪剧中
        lp.bottomMargin = rtImageBottom;
        imageView.setLayoutParams(lp);

//        GlideApp.with(getContext()).load(imagePath).centerCrop().placeholder(R.mipmap.img_load_fail).error(R.drawable.img_load_fail).into(imageView);
...
//		mTransitioner.setDuration(300);
    ...
    private void mergeEditText()
    {
        ...
        if (preView != null && preView instanceof EditText && null != nextView && nextView instanceof EditText)
        {
            ...
            preEdit.setText(SpanStringUtils.getEmotionContent(EmotionUtils.EMOTION_CLASSIC_TYPE, getContext(), preEdit, mergeText));
            preEdit.requestFocus();
     ...
     * 对外提供的接口, 生成编辑数据上传
     */
    public List<EditData> buildEditData(boolean removed)
    {
        ...
        for (int index = 0; index < num; index++)
        {
            ...
            if (itemView instanceof EditText)
            {
                ...
                if (removed)
                {
                    itemData.inputStr = item.getText().toString().replaceAll("\\s*", "");
                }
                else
                {
